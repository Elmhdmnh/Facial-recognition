...
识别网络 `w600k_r50` 使用 **ArcFace(Additive Angular Margin Loss)** 损失函数训练:在分类 Softmax 的基础上,对特征向量与类中心权重之间的**夹角加上固定 margin `m`**,迫使模型学到类内更紧凑、类间更分散的特征。正是这个设计让同一人的不同照片在余弦空间中天然聚集——这也是第 3 节选择余弦相似度做比对的根本原因。
